...
def apply_knn_adjustment(filtered_df, final_xg_model, final_xg_extreme_model, logistic_model, weighted_predictions,
                         start_date, k=5):
    """Apply kNN adjustment layer to improve temperature predictions."""
    # Create adjustment range
    adjustment_range = pd.date_range(start=start_date + pd.Timedelta(hours=1), periods=168, freq='h')
    adjustment_df = pd.DataFrame({'date': adjustment_range})
    adjustment_df['hour'] = adjustment_df['date'].dt.hour
    adjustment_df['day_of_year'] = adjustment_df['date'].dt.day_of_year
    adjustment_df['timestamp'] = adjustment_df['date'].apply(lambda x: x.timestamp())

    # Add Fourier features
    adjustment_df = add_fourier_features(adjustment_df, 'timestamp', k)

    # Prepare adjustment features
    adjust_features = pd.concat([
        adjustment_df[['hour', 'day_of_year']],
        adjustment_df.filter(like='sin_'),
        adjustment_df.filter(like='cos_')
    ], axis=1)

    # Predictions from models
    xg_y_adj_pred = final_xg_model.predict(adjust_features)
    xg_y_extreme_adj_pred = final_xg_extreme_model.predict(adjust_features)
    adj_extreme_probabilities = logistic_model.predict_proba(adjust_features.iloc[:, 2:])[:, 1]
    adj_weighted_predictions = (adj_extreme_probabilities * xg_y_extreme_adj_pred +
                                (1 - adj_extreme_probabilities) * xg_y_adj_pred)

    # Calculate errors for kNN training
    adjustment_actuals = filtered_df[filtered_df['date'] > start_date].copy()
    adjustment_actuals['error'] = adjustment_actuals['temperature'] - adj_weighted_predictions

    # Add Fourier features to training data
    adjustment_actuals = add_fourier_features(adjustment_actuals, 'timestamp', k)

    # Prepare training features for kNN
    train_features = pd.concat([
        adjustment_actuals[['hour', 'day_of_year']],
        adjustment_actuals.filter(like='sin_'),
        adjustment_actuals.filter(like='cos_')
    ], axis=1)
    train_target = adjustment_actuals['error']

    # Scale features
    scaler = StandardScaler()
    train_features_scaled = scaler.fit_transform(train_features)
    adjust_features_scaled = scaler.transform(adjust_features)

    # Train kNN model
    knn = KNeighborsRegressor(n_neighbors=5)
    knn.fit(train_features_scaled, train_target)

    # Predict error adjustments using kNN
    adjustment_df['knn_adjusted_predictions'] = adj_weighted_predictions + knn.predict(adjust_features_scaled)

    # Merge predictions and calculate errors
    adjustment_actuals = adjustment_actuals.merge(
        adjustment_df[['date', 'knn_adjusted_predictions']],
        on='date',
        how='left'
    )
    adjustment_actuals['knn_adjusted_error'] = (
            adjustment_actuals['temperature'] - adjustment_actuals['knn_adjusted_predictions']
    )

    # Add final predictions to future data
    max_date = filtered_df['date'].max()
    future_dates = pd.date_range(start=max_date + pd.Timedelta(hours=1), periods=168, freq='h')
    ...
    future_df['final_pred'] = weighted_predictions + knn.predict(adjust_features_scaled)

    return adjustment_actuals, future_df
########################################################################################################################
# Main Workflow
########################################################################################################################
...
def main():
    ...
    bottom_threshold = np.percentile(filtered_df['temperature'], 1.5)
    temp_extremes_low = filtered_df[filtered_df['temperature'] <= bottom_threshold]

    upper_threshold = np.percentile(filtered_df['temperature'], 98.5)
    ...
    # 7. Train kNN adjustment layer
    start_date = filtered_df['date'].max() - pd.Timedelta(hours=168)
    adjustment_actuals, future_df = apply_knn_adjustment(
        filtered_df=filtered_df,
        final_xg_model=final_xg_model,
        final_xg_extreme_model=final_xg_extreme_model,
